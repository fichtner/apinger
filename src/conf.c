...
void *
pool_malloc(struct pool_item **pool, size_t size)
{
	struct pool_item *pi;
	char *p;

	p = calloc(1, size + sizeof(struct pool_item));
...
char *
pool_strdup(struct pool_item **pool, const char *str)
{
	char *p;

	assert(str != NULL);

	p = pool_malloc(pool, strlen(str) + 1);
	strcpy(p, str);
...
void
pool_free(struct pool_item **pool, void *ptr)
{
...
void
pool_clear(struct pool_item **pool)
{
	struct pool_item *pi, *pi1;
...
void
add_alarm(enum alarm_type type)
{
	cur_alarm->type = type;
...
struct alarm_list *
alarm2list(const char *aname, struct alarm_list *list)
{
	struct alarm_cfg *ac;
	struct alarm_list *al;

	for (ac = cur_config.alarms; ac != NULL; ac = ac->next) {
		if (strcmp(ac->name, aname) == 0) {
			break;
		}
	}
